/**
 * Direct Anthropic API call from the mobile app.
 * Used when the backend is unreachable (e.g. no tunnel, iOS dev).
 * Requires EXPO_PUBLIC_ANTHROPIC_API_KEY in mobile/.env.
 *
 * NOTE: For development only. In production, use the backend to keep keys secret.
 */

import * as FileSystem from 'expo-file-system';

const ANTHROPIC_KEY = process.env.EXPO_PUBLIC_ANTHROPIC_API_KEY;
const ANTHROPIC_MODEL = process.env.EXPO_PUBLIC_ANTHROPIC_MODEL || 'claude-sonnet-4-6';
...
async function attachmentsToContentBlocks(attachments) {
  const blocks = [];
  if (!Array.isArray(attachments)) return blocks;
  for (const att of attachments.slice(0, 5)) {
    if (!att?.uri || !isImageFile(att.uri)) continue;
    try {
      const base64 = await FileSystem.readAsStringAsync(att.uri, { encoding: FileSystem.EncodingType.Base64 });
      blocks.push({
        type: 'image',
        source: { type: 'base64', media_type: guessMediaType(att.uri), data: base64 },
      });
    } catch (_) {}
  }
  return blocks;
...
// ──────────────────────────────────────────────
// Output format rules per display mode
// ──────────────────────────────────────────────
const OUTPUT_FORMAT = {
...
// ──────────────────────────────────────────────
// System prompt with advanced pedagogy
// ──────────────────────────────────────────────
...
export function isDirectAvailable() {
  return !!ANTHROPIC_KEY;
}

export async function solveDirect({ question, subject, output_preference, attachments }) {
  if (!ANTHROPIC_KEY) {
    throw new Error('Set EXPO_PUBLIC_ANTHROPIC_API_KEY in mobile/.env for direct AI calls.');
  }

  const subj = subject || 'Other';
  const outputPref = output_preference || 'handwritten';
  const subjectRule = SUBJECT_PROMPTS[subj] || SUBJECT_PROMPTS.Other;
  const formatRule = OUTPUT_FORMAT[outputPref] || OUTPUT_FORMAT.handwritten;

  const userText = `${subjectRule}\n${formatRule}\n\n<task>\nSubject: ${subj}\n\nQuestion:\n${question.trim()}\n</task>`;
  ...
  let answerText = textBlock?.text ?? 'No response generated.';
  answerText = postProcessAnswer(answerText);
...
/**
 * Streaming version of solveDirect.
 * Uses XMLHttpRequest (works in React Native) to read Anthropic SSE incrementally.
 */
export function solveDirectStream(
  { question, subject, output_preference, attachments },
  { onChunk, onDone, onError }
) {
  if (!ANTHROPIC_KEY) {
    onError?.(new Error('Set EXPO_PUBLIC_ANTHROPIC_API_KEY in mobile/.env for direct AI calls.'));
  ...
  return attachmentsToContentBlocks(attachments).then((imageBlocks) => {
    const userContent = imageBlocks.length > 0
      ? [...imageBlocks, { type: 'text', text: userText }]
      : userText;
    ...
    return new Promise((resolve) => {
      ...
      xhr.setRequestHeader('x-api-key', ANTHROPIC_KEY);
      ...
      xhr.onreadystatechange = () => {
        ...
        if (xhr.readyState === 4) {
          if (xhr.status >= 200 && xhr.status < 300) {
            const answerText = postProcessAnswer(accumulated);
            onDone?.({
              question: question.trim(),
              subject: subj,
              answerText,
            ...
            let msg = `Anthropic API error (${xhr.status})`;
            try {
      ...
      xhr.onerror = () => {
        onError?.(new Error('Network error calling Anthropic API'));
        resolve();
      };
