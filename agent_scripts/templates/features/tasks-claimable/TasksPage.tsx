...
import { useUser, useUsers, useQuery, useMutations } from '@spaces/sdk/storage'
// After copying to src/pages/, these imports will work:
import { Button, Modal, Badge, Avatar, EmptyState, type BadgeProps } from '../components/ui'
import { ROLES, type Role } from '../constants'
import { DIFFICULTY, DIFFICULTY_CONFIG, GRADE, GRADE_CONFIG, type Difficulty, type Grade } from '../constants/tasks-constants'

// Tailwind color classes for buttons (semantic colors)
type BadgeVariant = BadgeProps['variant']
const buttonColorClasses: Record<BadgeVariant, string> = {
...
function CreateChallengeModal({ isOpen, onClose, onCreate }: CreateChallengeModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Create Challenge" maxWidth="max-w-lg">
          ...
          <div className="flex gap-2">
            {Object.entries(DIFFICULTY_CONFIG).map(([key, config]) => (
              <button
                ...
                className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all border ${buttonColorClasses[config.color]} ${
                  difficulty === key
...
function SubmitModal({ isOpen, onClose, onSubmit, challenge }: SubmitModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Submit Challenge" maxWidth="max-w-lg">
      <div className="space-y-4">
...
function GradeModal({ isOpen, onClose, onGrade, challenge }: GradeModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Grade Submission" maxWidth="max-w-lg">
      <div className="space-y-4">
        <div className="p-3 bg-muted/40 rounded-lg border border-border/30">
          <h4 className="font-medium text-foreground">{challenge.data.title}</h4>
