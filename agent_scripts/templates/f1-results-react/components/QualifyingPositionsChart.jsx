...
/**
 * Chart showing driver qualifying positions over the season
 */
const QualifyingPositionsChart = ({ year, qualifyingStore }) => {
  const seasonQualifying = qualifyingStore?.[year] || {};
  const rounds = Object.keys(seasonQualifying).map(Number).sort((a, b) => a - b);
  ...
  if (rounds.length === 0) {
    return (
      <div style={{ textAlign: 'center', padding: 40, color: '#94a3b8' }}>
        No qualifying data available. Use "Preload All Data" to fetch qualifying results.
      </div>
    );
  }
  ...
  rounds.forEach(round => {
    const qualifying = seasonQualifying[round];
...
export default QualifyingPositionsChart;
